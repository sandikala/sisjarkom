...
import {renderResearch,bindResearch,initResearch,researchDirty,clearTimer} from './research-lite.js';
...
try{const r=await fetch(new URL('./course.json',import.meta.url),{cache:'no-store'});if(!r.ok)throw Error('Materi belum dapat dimuat.');modules=await r.json();await render();}catch(e){main.innerHTML=`<div class="workspace"><div class="info warning"><b>Ruang belajar belum dapat dimuat.</b><p>${esc(e.message)}</p><p>Jika membuka berkas langsung, jalankan melalui server lokal atau GitHub Pages. Pastikan folder assets-v21 ikut diunggah.</p><button class="button" id="retry-app">Muat ulang</button></div></div>`;document.getElementById('retry-app').onclick=()=>location.reload();}
